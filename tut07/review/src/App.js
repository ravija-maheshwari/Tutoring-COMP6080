...
function App() {
  ...
  const [num, setData] = useState(initialBoard);
  ...
  const goTopBottom = (direction) => {
    ...
    let newData = [...Array(4)].map((e) => Array(4).fill(null));
      ...
      <div className="row">
        {[0, 1, 2, 3].map((i) => (
          <div
            className="box"
            style={{
              background: changeColor(num[0][i]),
              color: fontColor(num[0][i]),
              fontSize: font(num[0][i]),
            }}
          >
            {num[0][i]}
          </div>
